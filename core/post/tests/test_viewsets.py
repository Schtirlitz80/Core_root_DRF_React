...
class TestPostViewSet:
    ...
    def test_list_anonimous(self, client, post):
        response = client.get(self.endpoint)
        assert response.status_code == status.HTTP_200_OK
        assert response.data["count"] == 1
    
    def test_retrieve_anonimous(self, client, post):
        response = client.get(self.endpoint + str(post.public_id) + "/")
        assert response.status_code == status.HTTP_200_OK
        assert response.data['id'] == post.public_id.hex
        assert response.data['body'] == post.body
    ...
    def test_create_anonimous(self, client):
        data = {
            "body": "Test Post Body",
            "author": "test_user"
        }
    ...
    def test_update_anonimous(self, client, post):
        data = {
            "body": "Test Post Body",
            "author": "test_user"
        }
    ...
    def test_delete_anonimous(self, client, post):
        response = client.delete(self.endpoint + str(post.public_id) + "/")
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
